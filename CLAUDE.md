...
```
- **Unit tests** cover pure-logic modules with no DOM dependency: `maintenanceStatus.js` (`computeStatus` — the status source of truth), `date.js`, `string.js`. Modules that import `lib/logger.js` can't be unit-tested this way — it touches `document`/`window` at module scope (real DOM elements, `window.onerror`), so importing it outside a browser throws. Those are covered by e2e tests instead, not by e.g. jsdom-stubbing logger.js's DOM calls.
...
- Local (not server-pushed) notifications for due items: footer button, throttled to once/day. See `js/lib/notifications.js`.
- A real automated test suite: `frontend/tests/unit` (Node's built-in test runner, pure-logic modules) + `frontend/tests/e2e` (Playwright, full browser flows). 23 unit + 14 e2e tests, all passing. See the "Tests" section above for how to run them and the headless-notification-permission/server-warm-up gotchas already solved there.
...
UX feedback flagged for the vehicle-summary/footer buttons (icon-only redesign, mileage-click-to-edit) — see the user's most recent ask; the "Export"/"Import" footer button labels are currently in English, inconsistent with the rest of the UI's Spanish text (flagged, not yet resolved either way).
