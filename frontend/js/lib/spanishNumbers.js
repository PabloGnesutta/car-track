/**
 * Parses spoken/dictated Spanish text into an integer — built for odometer
 * readings ("cincuenta mil trescientos kilómetros" -> 50300). Kept
 * dependency-free and pure so it's unit-testable without a browser (see
 * tests/unit/spanishNumbers.test.js); the Web Speech API wrapper that
 * feeds it live transcripts lives in lib/speechRecognition.js instead.
 *
 ...
 *  - Spanish number words ("cincuenta mil trescientos")
 */
...
/** Connectors/units that carry no numeric value and should be ignored. */
const SKIP_WORDS = new Set(['y', 'kilometros', 'kilometro', 'km', 'kms', 'de']);
...
function parseSpanishNumber(text) {
  ...
  for (const word of words) {
    ...
    // Unrecognized words (background noise, filler like "el auto", etc.)
