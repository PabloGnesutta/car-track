...
/**
 * Enums
 * @typedef {'vehicles'|'maintenanceItems'|'serviceHistory'|'mileageHistory'|'fuelHistory'} ObjectStores
...
function onDbUpgradeNeeded() {
  ...
  if (!db.objectStoreNames.contains(_stores.vehicles)) {
    db.createObjectStore(_stores.vehicles, { autoIncrement: true });
  }

  if (!db.objectStoreNames.contains(_stores.maintenanceItems)) {
    const store = db.createObjectStore(
      _stores.maintenanceItems,
      { autoIncrement: true }
    );
    store.createIndex('vehicleKey', 'vehicleKey', { unique: false });
  }

  if (!db.objectStoreNames.contains(_stores.serviceHistory)) {
    const store = db.createObjectStore(
      _stores.serviceHistory,
      { autoIncrement: true }
    );
  ...
  if (!db.objectStoreNames.contains(_stores.fuelHistory)) {
    const store = db.createObjectStore(
      _stores.fuelHistory,
      { autoIncrement: true }
    );
